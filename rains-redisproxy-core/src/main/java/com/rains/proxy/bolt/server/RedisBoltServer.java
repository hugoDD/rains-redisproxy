 ...
 */
package com.rains.proxy.bolt.server;
...
import com.rains.proxy.bolt.protocol.RedisServerHandler;
import com.rains.proxy.bolt.remoting.RedisBoltServerRemoting;
...
public class RedisBoltServer extends AbstractRemotingServer {

    /** logger */
    private static final Logger                         logger                  = BoltLoggerFactory
                                                                                    .getLogger("RedisBoltServer");
    /** server bootstrap */
    ...
    /** rpc remoting */
    protected RedisBoltServerRemoting rpcRemoting;
     ...
     */
    public RedisBoltServer(boolean manageConnection) {
     ...
     * Otherwise {@link UnsupportedOperationException} will be thrown.
     */
    public RedisBoltServer(int port) {
        this(port, false);
    }
     ...
     * Otherwise {@link UnsupportedOperationException} will be thrown.
     */
    public RedisBoltServer(String ip, int port) {
        this(ip, port, false);
    }
     ...
     * @param manageConnection true to enable connection management feature
     */
    public RedisBoltServer(int port, boolean manageConnection) {
        super(port);
     ...
     * @param manageConnection true to enable connection management feature
     */
    public RedisBoltServer(String ip, int port, boolean manageConnection) {
        super(ip, port);
        /* server connection management feature enabled or not, default value false, means disabled. */
        if (manageConnection) {
            option(BoltServerOption.SERVER_MANAGE_CONNECTION_SWITCH, true);
     ...
     */
    public RedisBoltServer(int port, boolean manageConnection, boolean syncStop) {
...
//                pipeline.addLast("encoder", codec.newEncoder());
                ...
                pipeline.addLast("handler", new RedisServerHandler());
                if (extendedHandlers != null) {
    ...
    protected void initRpcRemoting() {
        this.rpcRemoting = new RedisBoltServerRemoting(new RpcCommandFactory(), this.addressParser,
            this.connectionManager);
    }

    /**
     * @see RemotingServer#registerProcessor(byte, CommandCode, RemotingProcessor)
     */
    @Override
     ...
     *
     * @see AbstractRemotingServer#registerUserProcessor(UserProcessor)
     */
    @Override
    ...
    public boolean isConnected(String remoteAddr) {
        ensureStarted();
        Url url = this.rpcRemoting.getAddressParser().parse(remoteAddr);
    ...
    public boolean isConnected(Url url) {
        ensureStarted();
        Connection conn = this.rpcRemoting.getConnectionManager().get(url.getUniqueKey());
        if (null != conn) {
            return conn.isFine();
        }
        return false;
