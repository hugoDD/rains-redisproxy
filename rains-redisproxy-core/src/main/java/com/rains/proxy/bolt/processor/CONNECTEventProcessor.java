...
public class CONNECTEventProcessor implements ConnectionEventProcessor {
    ...
    private void doCheckConnection(Connection conn) {
        Assert.notNull(conn);
        Assert.notNull(conn.getPoolKeys());
        Assert.isTrue(conn.getPoolKeys().size() > 0);
        Assert.notNull(conn.getChannel());
        Assert.notNull(conn.getUrl());
        Assert.notNull(conn.getChannel().attr(Connection.CONNECTION).get());
    }
