...
public class BoltServer {
    ...
    /** rpc server */
    private RedisBoltServer server;

    // ~~~ constructors
    public BoltServer(int port) {
        this.port = port;
        this.server = new RedisBoltServer(this.port);
    }

    public BoltServer(int port, boolean manageFeatureEnabled) {
        this.port = port;
        this.server = new RedisBoltServer(this.port, manageFeatureEnabled);
    }

    public BoltServer(int port, boolean manageFeatureEnabled, boolean syncStop) {
        this.port = port;
        this.server = new RedisBoltServer(this.port, manageFeatureEnabled, syncStop);
    }
    ...
    public RedisBoltServer getRpcServer() {
        return this.server;
    }
