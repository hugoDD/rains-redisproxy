...
public class LBRedisServerHandler extends SimpleChannelInboundHandler<RedisCommand> {
    ...
    public void channelReadComplete(ChannelHandlerContext ctx) throws Exception {
        if (logger.isDebugEnabled()) {
            logger.debug("channelReadComplete : {}",ctx.channel().flush());
        }
    ...
    protected void channelRead0(ChannelHandlerContext ctx, RedisCommand request) throws Exception {
        Assert.notNull(request, "handle request command must not null");
        if (logger.isDebugEnabled()) {
            logger.debug("handle request command : {}", request.toString());
        }


        String command = new String(request.getArgs().get(0));
        IRedisReply redisReply = redisCommandControl.action(request,ctx);
